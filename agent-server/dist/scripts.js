    ...
    for (let i = 0; i < 64; i++) {
        ...
        for (let r = 0; r < 3; r++) {
            ...
            for (let c = 0; c < Math.min(numColumns, 8); c++) {
                ...
                if (c === 0)
                    row[key] = `${tableName}_${r}_${Math.random().toString(36).slice(2, 10)}`;
                else if (c === 1)
                    row[key] = `Sample ${domain} record ${r} with extended name for display`;
                else if (c === 2)
                    row[key] = ["active", "inactive", "pending"][r];
                else if (c === 3 || c === 4)
                    row[key] = new Date(2025, 0, 1 + r * 30).toISOString();
                else if (c === 5)
                    row[key] = Math.round(Math.random() * 10000) / 100;
                else if (c === 6)
                    row[key] = `ref_${(i + r) % 64}_${Math.random().toString(36).slice(2, 8)}`;
                else if (c === 7)
                    row[key] = { source: domain, version: r + 1, tags: ["auto", "generated"] };
            }
            sampleRows.push(row);
        }
        tables.push({
...
export function selectScript(userMessage) {
    ...
    if (!defaultScript)
        throw new Error("No default script found");
